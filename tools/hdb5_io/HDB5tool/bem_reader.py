...
class NemohReader(_BEMReader):
    ...
    def _pre_read_nemoh_parameters(self,pyHDB, cal_file):
        ...
        with open(cal_file, 'r') as f:
            ...
            data = f.readline().split()[:3]  # v3.0.2 [1:4]
        ...
        with open(cal_file, 'r') as f:
            ...
            if pyHDB.has_kochin:
                ...
                if pyHDB.nb_bodies > 1:
                    print("warning : Wave drift model (Kochin) non-consistent with multibody configuration."
                          "Wave drift load applied to body 0 only.")
    ...
    def _read_nemoh_results(self, pyHDB):  # FIXME: methode de reader nemoh
        ...
        with open(bem_results_file, 'r') as f:
            data = np.asarray(f.read().split(), dtype=np.float64)
        ...
        added_mass_data_v2 = np.zeros((nb_forces, nb_dof, nw), dtype = np.float64)
        radiation_damping_data_v2 = np.zeros((nb_forces, nb_dof, nw), dtype = np.float64)
    ...
    def _read_kochin_function_file(self,pyHDB):
        ...
        def read(i):
            ...
            with open(filename, 'r') as f:
                data = f.read()
            arr = np.array(data.split(), dtype=np.float64).reshape((-1, 3))
